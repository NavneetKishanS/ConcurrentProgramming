...
public class AuctionHouse {
    ...
    BlockingQueue<AuctionOffer> auctionQueue;
    ...
    private static Thread makeAuctioneer(List<Thread> artists) {
        ...
        // a single auction is done like this:
            ...
            return new Thread(() -> {
                while (artists.stream().any(x -> x.isAlive())) {
                    int idx = getRandomBetween(0, nftIdx-1);
                    auctionQueue = new ArrayBlockingQueue<AuctionOffer>();
                    while (true) {
                        try {
